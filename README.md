...
attn = sigmoid(q @ keys.T / √k)     # (B, L, G, G) — независимые per-expert гейты
help_k = attn @ keys                # взвешенная сумма
trust = 1 - contra                  # доверие к коллективу
help_k = help_k · sigmoid(w_help) · trust
```

- **softmax → sigmoid**: каждый эксперт получает независимый гейт чтения (0..1),
  без нормировки по строке. Амплитуда help_k масштабируется числом открытых
  гейтов (до G×) — ranking loss стартует выше и сходится медленнее, но гейты
  не конкурируют и могут быть одновременно нулевыми (полный отказ от памяти).
...
Актуальные архитектурные изменения и статус обучения:
...
- **w_pred_scale_legacy удалён**: мёртвый параметр (не получал градиента, std=0).
  Модуляция ошибки — через `pred_scale_mod` из delta_var, без learnable параметра.
- **Cross-expert attention: softmax → sigmoid**: независимые per-expert гейты
  чтения из private memory (см. §9.4). Убрана конкуренция и ненулевое нормировочное
  давление — гейты могут быть нулевыми одновременно.

### Обучение (реальные данные, sigmoid-код)

D=2560, L=24, G=32, 86.39M параметров, vocab=50000. Colab T4.

- **CE: 11.27 → 5.3** (пол случайного угадывания 10.82 пройден — реальные данные)
- **val_ppl: 13233 → 268** (step 2563), val_loss 9.49 → 5.59
- **ranking: 242 → 146** (step 2750) и ускоряется — mirror реально учится
- **lr_adapt mult: 0.50 → 1.0** — полная скорость LR достигнута к step ~2475
- **Гейт-дифференцировка в 2–3× быстрее**, чем в random-data запуске
  (b_gate std 6×, tanh_bias ±0.53 к step 1631)

...
| Метрика | Значение |
|---|---|
| Шаг сохранения best | **9786** (проанализирован: WAKE-CANDIDATE, отчёт `best_9786_report.html`). Прогон: val 9.4035@9087 → 9.3663@9320 → 9.3310@9553 → **9.2989@9786** (новый best). |
| val_loss / ppl | 9.2989 / ~10913 |
| Зрелость `mat` | 0.716–0.755 (текущий 0.7504@9786 — устойчивый режим после прорыва) |
...
10.0216 (5126) → 9.9749 (5359) → 9.9283 (5592) → 9.8816 (5825) → 9.8366 (6058) → 9.7956 (6291) → 9.7516 (6524) → 9.7113 (6757) → 9.6820 (6990) → 9.6390 (7223) → *перестройка (7689=11.09, 7922=15.74)* → **9.5201 (8388) → 9.4821 (8621) → 9.4425 (8854) → 9.4035 (9087) → 9.3663 (9320) → 9.3310 (9553) → **9.2989 (9786)** — монотонно улучшается после прорыва.
...
Интерпретация: перестройка — **успешный прорыв** в лучший бассейн. Модель «переломала» старое устойчивое внутреннее пространство и нашла более обобщающее (val 9.44 вместо 9.64). Ствол оставался жив (CE < 15), watchdog не срабатывал. Обучение продолжается к цели `val ≈ 8.5`. Локальный `checkpoints/best.pt` = 9786 (свежий, проанализирован: WAKE-CANDIDATE, отчёт `best_9786_report.html`); после 8854 val продолжил снижаться **9.4425@8854 → 9.4035@9087 → 9.3663@9320 → 9.3310@9553 → 9.2989@9786**.
...
- Maturation gate 0.7504 (открыт, max 0.750 — устойчивый режим после прорыва); `bridge_readiness` max = 0.7844, `tau_norm` max = 0.996 — мост реально включается.
...
- HTML-отчёт: `checkpoints/best_9786_report.html`.

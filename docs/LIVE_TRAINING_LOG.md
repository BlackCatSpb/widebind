...
| Метрика | Значение |
|---|---|
| Шаг сохранения best | **10485** (проанализирован: WAKE-CANDIDATE, отчёт `best_10485_report.html`). Прогон: val 9.4035@9087 → 9.3663@9320 → 9.3310@9553 → 9.2989@9786 → 9.2646@10019 → 9.2307@10252 → **9.1963@10485** (новый best). |
| val_loss / ppl | 9.1963 / ~9861 |
| Зрелость `mat` | 0.716–0.755 (текущий 0.7501@10485 — устойчивый режим после прорыва) |
...
⏳ *Сырой лог за 8965 → 10615 получен (ниже); локальный `best.pt` = **10485** (val 9.1963, WAKE-CANDIDATE, отчёт `best_10485_report.html`). Steady-state держится: `mat` 0.750, CE трейна **7.36** (step 10450 — новый минимум), `intent_w`~2.14, `bridge_conn` raw ~0.026. Концептов стало больше: slots 91/192 (было 83), full layers 11 (было 10). Обучение продолжается к цели `val ≈ 8.5`.*

### Вердикт analyze (`checkpoints/best.pt`@10485) — WAKE-CANDIDATE

- MLP проснулся: `W_std` = 0.0703 ≈ decay-базис (0.0695, dev +0.0008); gate max 0.751 (порог WAKE 0.75, L7/L23/L5). `sigmoid(mod_scale_mlp)` = 0.749, `sigmoid(mod_scale_mem)` = 0.667.
- Maturation gate 0.7501 (открыт, max 0.750 — устойчивый режим после прорыва); `bridge_readiness` max = 0.7856, `tau_norm` max = 0.996 — мост реально включается.
...
- Концепты: slots 91/192, full layers 11, births в пустых слоях **[12, 13, 14]** (продолжается штатное рождение новых концептов, WATCH; заполненность выросла 83→91).
...
- HTML-отчёт: `checkpoints/best_10485_report.html`.

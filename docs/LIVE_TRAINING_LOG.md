...
- Maturation readiness выросла значительно: 0.444→0.576→0.677 (реди-reeNode растёт)
- Maturation gate медленно: 0.064→0.068→0.071 (рамп ~55 steps/0.001)
- mod_scale_mlp стабилен ~0.664 (baseline 0.668) — MLP gate не проснулся
- Memory bank: L1=L2=L3=0 во всех — mat < 0.3, ждём step ~8-10k
- Private memory norms: 0.025→0.028→0.030 — медленный рост
...
step  1165: val=10.8568 val_ppl=51,884.04  ← best 3.pt (current best)
```
...
**Фаза 3 (step 700–1265): Медленное снижение**
- CE: 10.84→10.74→10.76→10.78 — стабильный дрейф вниз
- val: 10.95→10.90→10.86 (лучший)
- Maturation deep: 0.138→0.155 — рамп продолжается
- Maturation readiness: 0.444→0.677 — значительный рост
...
| Метрика | best.pt (699) | best 2.pt (932) | best 3.pt (1165) | Дельта |
|---|---|---|---|---|
| val_loss | 10.9503 | 10.8955 | 10.8568 | -0.0935 |
| val_ppl | 56,973 | 53,934 | 51,884 | -5,089 |
| mat gate mean | 0.064 | 0.068 | 0.071 | +0.007 |
| mat readiness mean | 0.444 | 0.576 | 0.677 | +0.233 |
| mod_mlp mean | 0.664 | 0.664 | 0.664 | 0.000 |
| Memory Bank L1/L2/L3 | 0/0/0 | 0/0/0 | 0/0/0 | — |
| Private mem norm | 0.025 | 0.028 | 0.030 | +0.005 |
| NaN/Inf | 0/0 | 0/0 | 0/0 | — |

**Ключевой вывод:** Maturation readiness растёт быстрее mat gate (0.233 vs 0.007 за 466 шагов). Это значит, что слои "зреют" по readiness, но gate ещё не открылся. При текущей скорости readiness выйдет на 1.0 примерно на step ~2000, а gate достигнет 0.3 на step ~8000-10000.

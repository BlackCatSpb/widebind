...
| Метрика | Значение |
|---|---|
| Шаг сохранения best | **8854** (обучение продолжается, последний лог step 8965). Локальный `checkpoints/best.pt` = 8854 (свежий, проанализирован: WAKE-CANDIDATE). |
...
Интерпретация: перестройка — **успешный прорыв** в лучший бассейн. Модель «переломала» старое устойчивое внутреннее пространство и нашла более обобщающее (val 9.44 вместо 9.64). Ствол оставался жив (CE < 15), watchdog не срабатывал. Обучение продолжается к цели `val ≈ 8.5`. Локальный `checkpoints/best.pt` = 8854 (свежий, проанализирован: WAKE-CANDIDATE, отчёт `best_8854_report.html`).
...
### Вердикт analyze (`checkpoints/best.pt`@8854) — WAKE-CANDIDATE

- MLP проснулся: `W_std` = 0.0704 ≈ decay-базис (0.0697, dev +0.0007); gate max 0.751 (порог WAKE 0.75, L7/L23/L5). `sigmoid(mod_scale_mlp)` = 0.749, `sigmoid(mod_scale_mem)` = 0.667.
- Maturation gate 0.7167 (открыт, max 0.717 — новый устойчивый режим после прорыва, ниже пика 0.736 во время перестройки); `bridge_readiness` max = 0.7821, `tau_norm` max = 0.996 — мост реально включается.
...
- HTML-отчёт: `checkpoints/best_8854_report.html`.

...
| Метрика | Значение |
|---|---|
| Шаг сохранения best | **10019** (проанализирован: WAKE-CANDIDATE, отчёт `best_10019_report.html`). Прогон: val 9.4035@9087 → 9.3663@9320 → 9.3310@9553 → 9.2989@9786 → **9.2646@10019** (новый best). |
| val_loss / ppl | 9.2646 / ~10597 |
| Зрелость `mat` | 0.716–0.755 (текущий 0.7504@10019 — устойчивый режим после прорыва) |
...
10.0216 (5126) → 9.9749 (5359) → 9.9283 (5592) → 9.8816 (5825) → 9.8366 (6058) → 9.7956 (6291) → 9.7516 (6524) → 9.7113 (6757) → 9.6820 (6990) → 9.6390 (7223) → *перестройка (7689=11.09, 7922=15.74)* → **9.5201 (8388) → 9.4821 (8621) → 9.4425 (8854) → 9.4035 (9087) → 9.3663 (9320) → 9.3310 (9553) → 9.2989 (9786) → **9.2646 (10019)** — монотонно улучшается после прорыва.
...
Интерпретация: перестройка — **успешный прорыв** в лучший бассейн. Модель «переломала» старое устойчивое внутреннее пространство и нашла более обобщающее (val 9.44 вместо 9.64). Ствол оставался жив (CE < 15), watchdog не срабатывал. Обучение продолжается к цели `val ≈ 8.5`. Локальный `checkpoints/best.pt` = 10019 (свежий, проанализирован: WAKE-CANDIDATE, отчёт `best_10019_report.html`); после 8854 val продолжил снижаться **9.4425@8854 → 9.4035@9087 → 9.3663@9320 → 9.3310@9553 → 9.2989@9786 → 9.2646@10019**.
...
⏳ *Сырой лог за 8965 → 10010 получен (ниже); локальный `best.pt` = **10019** (val 9.2646, WAKE-CANDIDATE, отчёт `best_10019_report.html`). Зафиксирован устойчивый steady-state после прорыва: `mat` 0.750–0.751, CE трейна ~7.8–8.9 (step 9845 = 7.81 — новый минимум), `intent_w`~2.14, `bridge_conn` raw ~0.025 (уравновешен). Обучение продолжается к цели `val ≈ 8.5`.*

### Вердикт analyze (`checkpoints/best.pt`@10019) — WAKE-CANDIDATE
...
- Maturation gate 0.7504 (открыт, max 0.750 — устойчивый режим после прорыва); `bridge_readiness` max = 0.7848, `tau_norm` max = 0.996 — мост реально включается.
...
- HTML-отчёт: `checkpoints/best_10019_report.html`.

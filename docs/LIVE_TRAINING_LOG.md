...
| Метрика | Значение |
|---|---|
| Шаг сохранения best | **9320** (проанализирован: WAKE-CANDIDATE, отчёт `best_9320_report.html`). Прогон: val 9.4035@9087 → **9.3663@9320** (новый best). |
| val_loss / ppl | 9.3663 / ~11688 |
| Зрелость `mat` | 0.716–0.755 (текущий 0.7486@9320 — устойчивый режим после прорыва) |
...
Интерпретация: перестройка — **успешный прорыв** в лучший бассейн. Модель «переломала» старое устойчивое внутреннее пространство и нашла более обобщающее (val 9.44 вместо 9.64). Ствол оставался жив (CE < 15), watchdog не срабатывал. Обучение продолжается к цели `val ≈ 8.5`. Локальный `checkpoints/best.pt` = 9320 (свежий, проанализирован: WAKE-CANDIDATE, отчёт `best_9320_report.html`); после 8854 val продолжил снижаться **9.4425@8854 → 9.4035@9087 → 9.3663@9320**.
...
⏳ *Сырой лог за 8965 → 9350 получен (ниже); локальный `best.pt` обновлён до **9320** (val 9.3663, WAKE-CANDIDATE, отчёт `best_9320_report.html`). Обучение продолжается к цели `val ≈ 8.5`.*

### Вердикт analyze (`checkpoints/best.pt`@9320) — WAKE-CANDIDATE
...
- Maturation gate 0.7486 (открыт, max 0.749 — устойчивый режим после прорыва); `bridge_readiness` max = 0.7833, `tau_norm` max = 0.996 — мост реально включается.
...
- HTML-отчёт: `checkpoints/best_9320_report.html`.

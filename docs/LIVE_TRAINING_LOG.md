...
| Метрика | Значение |
|---|---|
| Локальный best (проанализирован) | **10485** (val 9.1963, WAKE-CANDIDATE, отчёт `best_10485_report.html`). |
| **Best прогона** ( Colab) | **8.7692@15844** (ppl ~6433). Локально НЕ скопирован. |
| Текущий step на прогоне | ≥19635 (лог обрезан). val после best пошёл вверх: 8.77→9.04. |
| Зрелость `mat` | 0.722–0.755; после 15844 mat ~0.722–0.725 (вторичная перестройка). |
| `bridge_conn` (raw) | ~0.08 (вырос с 0.026@10485 → 0.08@15600+; мост перекалибруется). |
| `intent_w` | ~2.5 (вырос с 2.14@10485; интенсивнее модулирует ствол). |

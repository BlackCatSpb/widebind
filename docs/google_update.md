...
### 9. Результаты после перезапуска (шаги 2000→6900, подтверждено)

| Метрика | Что было на step 2000 | Что стало на step 6900 |
|---|---|---|
| `val_loss` | 5.92 | **1.90** (−68%) |
| `train_loss` | 7.11 | **1.64** (−77%) |
| `\|1-alpha\|` | 0.0077 | **0.0147** (+91%) — стабильно >0.01 на всех 32 слоях |
| `gate_var` | 0.0017 | **0.091** (×53) — эксперты дифференцированы |
| `var(ls)` | 0.00771 | **0.0080** (+4%) — умеренный рост |
| `LR` | 2.65e-4 (0.88×) | **9.00e-4** (3.0×, ceiling) |
| `Lr scheduler mult` | 0.88 | **3.0** (упирается в ceiling) |

**Вывод:** Все ожидания подтверждены. Alpha Warmup Override + Bidirectional LR разрывают chicken-and-egg петлю. Эксперты специализируются, LR растёт, loss падает. Система в устойчивом режиме саморегуляции.

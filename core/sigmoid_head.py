...
Объединяет:
  ...
  D. Двухстадийное декодирование: forward возвращает все V логитов через
     ...
     а predict дает жадный argmax; для выборки — двухстадийная грубая аргуация
     поверх top-candidates без полной V-нормализации.
...
def _logit(p):
    """logit(p) с защитой от краёв."""
    p = p.clamp(1e-9, 1 - 1e-9)
    return torch.log(p) - torch.log1p(-p)


def _fibramp(n):
    """Fibonacci-slope ramp для инициализации температур (стилистический τ-приорат)."""
    vals = []
    a, b = 1.0, 1.0
    for _ in range(n):
        vals.append(a)
...
class SigmoidCodedHead(nn.Module):
    ...
    def _gates(self, h, temp_factor=None):
        """(h: (B, L, D)) -> zt: (B, L, K) недостаточно-калиброванные gate-логиты."""
        B, L, D = h.shape
    ...
    def predict(self, h, k=512):
        """Two-stage decode (D): фокусированный greedy argmax над top-k.
        Первый проход — плотная оценка по u-векторам; второй — точный argmax."""
        lg = self.forward(h)              # stage-1: exact factored log-probs

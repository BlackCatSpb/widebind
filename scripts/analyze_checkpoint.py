...
Importing this module has no side effects; run the analysis via
  analyze(path)  or  generate_report(path)  (alias used by train.py).
"""
import sys, os, json
...
def generate_report(save_path):
    """Report used by train.py after each save (no side effects on import)."""
    analyze(save_path)

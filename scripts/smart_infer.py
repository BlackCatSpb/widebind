"""
smart_infer.py — экспериментальный «умный инференс» для WideBind.

Контроллер SmartController сам подбирает параметры генерации ПЕРЕТОКЕНОВО,
опираясь на собственные сигналы модели:
...
Режимы: exploit (уверен, острый) / explore (выбор) / confused (растерян) /
reason (включает reasoning) / recover-rep / recover-collapse (восстановление).
"""
import os, sys, math, argparse, inspect, torch
...
# скалярные метакогнитивные ключи, по которым усредняем по слоям
...
class SmartController:
    ...
    def decide(self, logits, mind, step):
        ...
        self.model.reasoning_scale_override = reason
...
def smart_generate(model, prompt, controller, max_new_tokens=64):
...
def main():
    ...
    # Baseline: статические параметры
    model.reasoning_scale_override = 0.0
    base = generate(model, args.prompt, max_new_tokens=args.tokens,
                    temperature=0.9, top_k=0, rep_penalty=2.0, rep_window=5,
                    reset_reasoning=False, bias_alpha=0.0)
    print('MODE: baseline (static temp=0.9, top_p disabled, rep=2.0, reasoning off)')
    print('BASELINE:', base)
    print()

    # Smart: адаптивный контроллер
    ...
    print(f'[tau] personality={ctrl.tau_personality:.1f} norm={ctrl.tau_norm:.2f} '
          f'temp_bias=({ctrl.temp_lo:.2f},{ctrl.temp_hi:.2f}) reason_thr={ctrl.reason_thr:.2f}')
    smart, dec = smart_generate(model, args.prompt, ctrl, args.tokens)
    print('MODE: smart (per-token adaptive: temp/top_p/top_k/rep + reasoning toggle)')
    print('SMART:', smart)
    print()
